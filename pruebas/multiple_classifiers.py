...
def save(score):
	with open('scores_train_neightbors.csv', 'a') as csvfile2:
...
def calcScores(classifiers, data, target):
	scores = []
	for clf in classifiers:
		print("Multinomial alpha = :")
		this_scores = cross_validation.cross_val_score(clf, X_train_tfidf, target, cv = 3)
		print(this_scores)
		save(np.mean(this_scores))
	return scores

classifiers = [
	KNeighborsClassifier(n_neighbors = 5, weights = 'uniform', algorithm = 'kd_tree', metric='minkowski', p = 2,  n_jobs = 5)]
allScores = []
...
print('Scores SP')
allScores.append(['SP'] + calcScores(classifiers, X_train_tfidf, target))
...
print('Scores ES')
allScores.append(['ES'] + calcScores(classifiers, X_train_tfidf, target))
...
print('Scores MIN3')
allScores.append(['MIN3'] + calcScores(classifiers, X_train_tfidf, target))
...
allScores.append(['ES + MIN3'] + calcScores(classifiers, X_train_tfidf, target))
...
print('Scores STEAM')
allScores.append(['STEAM'] + calcScores(classifiers, X_train_tfidf, target))
...
print('Scores LEMMA')
allScores.append(['LEMMA'] + calcScores(classifiers, X_train_tfidf, target))
...
allScores.append(['ES + MIN3 + STEAM'] + calcScores(classifiers, X_train_tfidf, target))
...
allScores.append(['ES + MIN3 + LEMMA'] + calcScores(classifiers, X_train_tfidf, target))
